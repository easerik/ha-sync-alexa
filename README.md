...
#### Parte 1: Configuração na AWS (IAM, DynamoDB, Lambda)
...
4. **Permissões da Skill:**
   - Configure as permissões no **Amazon Developer Console**:
     - **Send Alexa Events:** Habilite para permitir que a skill envie mensagens assíncronas para o endpoint de eventos da Alexa, respondendo a diretivas e enviando eventos de mudança de estado.
     - **Alexa Skill Messaging:** Use o `clientId` e `clientSecret` para mensagens da skill, permitindo eventos fora de sessão (não iniciados pelo usuário).
       - **Alexa Client Id:** `amzn1.application-oa2-client.edf447576ac8496cb907f2da4d7e5d89`
       - **Alexa Client Secret:** `**************************************************************************************`
     - **Importante:** Após configurar as permissões, **desabilite a skill, aguarde 30 segundos e habilite novamente** para garantir que as alterações entrem em vigor.

#### Parte 2: Configuração no Home Assistant
...
#### Part 1: AWS Setup (IAM, DynamoDB, & Lambda)
...
4. **Skill Permissions:**
   - Configure permissions in the **Amazon Developer Console**:
     - **Send Alexa Events:** Enable to allow the skill to send asynchronous messages to the Alexa Inbound Event endpoint, responding to directives and sending state change events.
     - **Alexa Skill Messaging:** Use the `clientId` and `clientSecret` for skill messaging, enabling out-of-session (non-customer invoked) events to be sent to skill code.
       - **Alexa Client Id:** `amzn1.application-oa2-client.edf447576ac8496cb907f2da4d7e5d89`
       - **Alexa Client Secret:** `**************************************************************************************`
     - **Important:** After configuring permissions, **disable the skill, wait 30 seconds, and re-enable it** to ensure the changes take effect.
